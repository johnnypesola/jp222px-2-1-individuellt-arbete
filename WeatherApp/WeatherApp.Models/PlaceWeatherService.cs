...
namespace WeatherApp.Models
{
    public class PlaceWeatherService : IPlaceWeatherService, IDisposable
    {
        ...
        {
            IEnumerable<Weather> WeatherForPlaceList = GetDateWeatherForPlace(placeId, out placeName);

            if (WeatherForPlaceList == null)
            {
                throw new HandleableExeption("Platsen kunde inte hittas.");
            }

            return ConvertToDayForecasts(WeatherForPlaceList);
        }
